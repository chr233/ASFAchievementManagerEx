...
| `ASET [Bots] AppId <AchievementIds>`    | -    | `Master`   | Аналогично `AUNLOCK` (原版插件的指令).                                    |
| `ALOCK [Bots] AppId <AchievementIds>`   | -    | `Master`   | Заблокировать достижения для определенных игр, 部分成就只能由官方服务器设置, 客户端无法解锁 |
| `ARESET [Bots] AppId <AchievementIds>`  | -    | `Master`   | Аналогично `ALOCK` (原版插件的指令)                                      |
